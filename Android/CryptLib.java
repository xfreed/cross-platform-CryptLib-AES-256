...
//     */
    ...
    public static String EnCrypt(String plainText,String iv,String key) {
        try
        {
            ...
            String result = crypto.encrypt(plainText, key, iv) + iv;
            return result;
        }
        catch (Exception e){
            e.printStackTrace();
    ...
    public static String DeCrypt(String plainText,String key) {
        int end = plainText.length();
        int start = end-16;
        String iv = plainText.substring(start,end);
        plainText = plainText.replace(iv, "");
        try
        {
            key = CryptLib.SHA256(key, 32);
            CryptLib crypto = new CryptLib();
            String result = crypto.decrypt(plainText, key, iv);
            return result;
        }
        catch (Exception e){
            e.printStackTrace();
